import { getState, setState, showToast, getStreak, getAvg, fmtDate, sanitizeInput } from '../utils/helpers.js';
...
window.showCheckinForm = function() {
...
function autoPost(data) {
  ...
  state.feedPosts.unshift({
    ...
    color: ['#5B3FF8','#00C896','#FF9F1C','#3B82F6'][Math.floor(Math.random()*4)], text: `Just completed check-in #${state.checkins.length}! 💪${wins?' Today: '+wins+'.':''} Staying consistent.`,
