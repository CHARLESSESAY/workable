import { initFirebase, onAuthStateChanged, signInWithEmail, signInWithGoogle, signOut } from './firebase/auth.js';
import { initServices } from './services/ai.js';
import { launch, restoreState } from './features/onboarding.js';
import { goTo, showToast, getState, setState, renderSkeleton } from './utils/helpers.js';
import { loadState, saveState } from './utils/localStorage.js';
import { handleError } from './utils/errorHandler.js';

// Make essential functions global so inline onclick handlers work
window.goTo = goTo;
...
// Global navigation handler (called by sidebar/mobile buttons)
window.nav = async function(id) {
  ...
  if (id === 'mytrack') import('./features/onboarding.js');
  ...
  if (id === 'ai') import('./features/onboarding.js');
};
...
window.doLogin = async function() {
  ...
  try {
    ...
    if (ok) launch();
    else showToast('⚠️', 'Profile not found. Please sign up.');
...
window.sbSignOut = async function() {
  await signOut();
  setState({...DEFAULT_STATE}); // DEFAULT_STATE will be imported from onboarding
  saveState();
  goTo('landing');
